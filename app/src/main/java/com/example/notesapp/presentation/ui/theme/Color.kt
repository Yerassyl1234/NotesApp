package com.example.notesapp.presentation.ui.theme

import androidx.compose.ui.graphics.Color

val White = Color(0xFFFFFFFF)

val Yellow100 = Color(0xFFFDF2E1)
val Yellow200 = Color(0xFFFCE5C3)
val Blue100 = Color(0xFFE3EBF2)
val Blue200 = Color(0xFFC8D8E5)
val Green = Color(0xFFD1EAD1)
val Purple = Color(0xFFDACEE6)

val Grey100 = Color(0xFFF9F9F9)
val Grey200 = Color(0xFF868484)
val Grey300 = Color(0xFF3D3B3B)


val Orange = Color(0xFFFFE0B2)
val Pink = Color(0xFFF8BBD0)
val Teal = Color(0xFFB2DFDB)

val LightOrange = Color(0xFFFFF3E0)
val LightPink = Color(0xFFFCE4EC)
val LightTeal = Color(0xFFE0F2F1)

val Brown = Color(0xFF703410)

val PinnedNotesColors = listOf(
    Yellow200, Purple, Blue200,
    Orange, Pink, Teal
)

val OtherNotesColors = listOf(
    Yellow100, Green, Blue100,
    LightOrange, LightPink, LightTeal
)